...
class EmailChannelService
{
    ...
    public function testConnection(EmailChannel $channel): array
    {
        if (!$channel->gmail_access_token) {
            return ['success' => false, 'message' => 'Canal no autorizado. Completa el flujo OAuth primero.'];
        }

        try {
            $response = \Illuminate\Support\Facades\Http::withToken(
                json_decode($channel->gmail_access_token, true)['access_token'] ?? ''
    ...
    /**
     * Process a raw RFC 2822 email string received from Gmail API.
     */
